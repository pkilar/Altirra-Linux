...
#ifndef AT_DISPLAY_WX_H
#define AT_DISPLAY_WX_H
...
// GL canvas widget — handles wxWidgets events and GL context
class ATDisplayCanvas : public wxGLCanvas {
public:
	ATDisplayCanvas(wxWindow *parent);

	wxGLContext *GetGLContext() const { return mpGLContext; }
	bool InitGL();
	void ShutdownGL();

	// Back-pointer so OnPaint can trigger GL rendering
...
private:
	...
	wxGLContext *mpGLContext = nullptr;
	ATDisplayWx *mpDisplay = nullptr;

	wxDECLARE_EVENT_TABLE();
};

// Display implementation — implements IVDVideoDisplay, owns GL texture,
// delegates GL context and swap to ATDisplayCanvas.
class ATDisplayWx final : public IVDVideoDisplay {
public:
	explicit ATDisplayWx(ATDisplayCanvas *canvas);
	~ATDisplayWx();

	ATDisplayCanvas *GetCanvas() const { return mpCanvas; }
