...
////////////////////////////////////////////////////////////////////////////////
// ATMidiOutput — Linux ALSA RawMidi implementation
...
private:
	...
	snd_rawmidi_t *mpRawMidi {};

	uint8 mSysExBuffer[256] {};
};

ATMidiOutput::ATMidiOutput() {
	// Try virtual MIDI port first, then default hardware
	int err = snd_rawmidi_open(nullptr, &mpRawMidi, "virtual", SND_RAWMIDI_NONBLOCK);
	if (err < 0) {
		err = snd_rawmidi_open(nullptr, &mpRawMidi, "default", SND_RAWMIDI_NONBLOCK);
		if (err < 0) {
			err = snd_rawmidi_open(nullptr, &mpRawMidi, "hw:0,0", SND_RAWMIDI_NONBLOCK);
			if (err < 0)
				mpRawMidi = nullptr;
		}
	}
...
ATMidiOutput::~ATMidiOutput() {
	if (mpRawMidi) {
		snd_rawmidi_drain(mpRawMidi);
		snd_rawmidi_close(mpRawMidi);
		mpRawMidi = nullptr;
	}
}
...
void ATMidiOutput::Reset() {
	...
	if (mpRawMidi) {
		// Send All Notes Off (CC 123, value 0) on all 16 channels
		for (int ch = 0; ch < 16; ++ch) {
			uint8 msg[3] = { (uint8)(0xB0 | ch), 123, 0 };
			snd_rawmidi_write(mpRawMidi, msg, 3);
		}
		snd_rawmidi_drain(mpRawMidi);
	}
}
...
void ATMidiOutput::ProcessDataByte(uint8 c) {
	switch(mState) {
		...
		case 2:
			...
			mState = 0;
			SendShortMsg(mMsgStatus + ((uint32)mMsgData1 << 8) + ((uint32)mMsgData2 << 16));
			break;

		case 3:
			mMsgData1 = c;
			g_ATLCMIDI("Message out: %02X %02X\n", mMsgStatus, mMsgData1);
			SendShortMsg(mMsgStatus + ((uint32)mMsgData1 << 8));
			mState = 0;
			break;
	}
}

void ATMidiOutput::SendShortMsg(uint32 msg) {
	if (!mpRawMidi)
		return;

	// Unpack the short message into raw MIDI bytes
	uint8 buf[3];
	int len = 1;
	buf[0] = (uint8)(msg & 0xFF);

	uint8 status = buf[0] & 0xF0;
	if (status == 0xC0 || status == 0xD0 || buf[0] >= 0xF4) {
		// 1-byte message (program change, channel pressure, some system)
		if (buf[0] < 0xF4) {
			buf[1] = (uint8)((msg >> 8) & 0xFF);
			len = 2;
		}
	} else if (buf[0] < 0xF0 || buf[0] == 0xF1 || buf[0] == 0xF2) {
		// 2 or 3 byte message
		buf[1] = (uint8)((msg >> 8) & 0xFF);
		buf[2] = (uint8)((msg >> 16) & 0xFF);
		len = (status == 0xC0 || status == 0xD0) ? 2 : 3;
	}

	snd_rawmidi_write(mpRawMidi, buf, len);
}

////////////////////////////////////////////////////////////////////////////////
// ATDeviceMidiMate — platform-independent device logic
